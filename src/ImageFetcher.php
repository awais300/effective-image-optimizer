...
class ImageFetcher extends Singleton
{
    ...
    public function get_attachment_images_xx($attachment_id)
    {
        // Start with the requested attachment
        $images = [];
        $processed_ids = [$attachment_id]; // Track processed IDs to avoid duplicates
        $ids_to_process = [$attachment_id]; // Queue of IDs to process
        
        // Process all queued attachments (non-recursively)
        while (!empty($ids_to_process)) {
            ...
            // Get both full and original paths
            $full_path = get_attached_file($current_id);
            $original_path = function_exists('wp_get_original_image_path') ? wp_get_original_image_path($current_id) : null;
            ...
            if ($setting_thumbnail_compression !== 'no') {
                ...
                // Allow filtering of excluded thumbnail sizes
                $excluded_sizes = apply_filters('awp_image_optimizer_excluded_thumbnails', $excluded_sizes, $current_id);
            ...
            // If this is the first attachment being processed (the one originally requested)
            // and WooCommerce is active, check for related product gallery images
            if ($current_id === $attachment_id && class_exists('WooCommerce')) {
                // Get the post associated with the attachment
                $post_id = wp_get_post_parent_id($current_id);
                ...
                if ($post_id && 'product' === get_post_type($post_id)) {
                    ...
                    if ($product) {
                        ...
                        // Add product thumbnail if it exists and isn't already processed
                        $product_image_id = $product->get_image_id();
                        if ($product_image_id && !in_array($product_image_id, $processed_ids)) {
                            $ids_to_process[] = $product_image_id;
                            $processed_ids[] = $product_image_id;
                        }
                        
                        // Add gallery images to processing queue if not already processed
                        foreach ($gallery_image_ids as $gallery_id) {
                            if (!in_array($gallery_id, $processed_ids)) {
                                $ids_to_process[] = $gallery_id;
                                $processed_ids[] = $gallery_id;
                            }
                        }
                    }
                }
